...
//
// Status: partially wired foundation.
//   - Live in this repo: cmd/worker registers an outbox publisher when NATS_URL is set; streams are
//     ensured at startup; when NATS_URL is empty, outbox rows are never published to the broker.
...
//
// Environment: set NATS_URL (e.g. nats://127.0.0.1:4222) for the worker outbox publisher; omit to
// leave outbox dispatch DB-only from the broker’s perspective.
package nats
