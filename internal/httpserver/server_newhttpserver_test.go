...
func TestNewHTTPServer_production_metrics_opsListenerRequiresBearerWhenTokenSet(t *testing.T) {
	...
	req.Header.Set("Authorization", "Bearer wrong-token-not-valid-xx")
	recWrong := httptest.NewRecorder()
	hs.ops.Handler.ServeHTTP(recWrong, req)
	require.Equal(t, http.StatusUnauthorized, recWrong.Code)
	req2 := httptest.NewRequest(http.MethodGet, "/metrics", nil)
	req2.Header.Set("Authorization", "Bearer ops-metrics-token-16b")
	rec2 := httptest.NewRecorder()
	hs.ops.Handler.ServeHTTP(rec2, req2)
	require.Equal(t, http.StatusOK, rec2.Code)
