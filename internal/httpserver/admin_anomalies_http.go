...
func serveAdminOperationalAnomaliesList(app *api.HTTPApplication) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		rows, err := app.Anomalies.List(r.Context(), scopeID, nil, int32(limit), int32(offset), refresh)
		if err != nil {
...
func serveAdminOperationalAnomalyGet(app *api.HTTPApplication) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		row, err := app.Anomalies.Get(r.Context(), scopeID, anomalyID)
		if err != nil {
...
func serveAdminOperationalAnomalyResolve(app *api.HTTPApplication) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		err = app.Anomalies.Resolve(r.Context(), scopeID, anomalyID, actor, body.Note)
		if err != nil {
		...
		fleetAudit(r.Context(), app, scopeID, "admin.operational_anomaly.resolve", "inventory_anomalies", stringPtrOrNil(anomalyID.String()), map[string]any{"note": body.Note})
...
func serveAdminOperationalAnomalyIgnore(app *api.HTTPApplication) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		err = app.Anomalies.Ignore(r.Context(), scopeID, anomalyID, actor, body.Note)
		if err != nil {
			if errors.Is(err, anomalies.ErrNotFoundOpen) || errors.Is(err, pgx.ErrNoRows) {
		...
		fleetAudit(r.Context(), app, scopeID, "admin.operational_anomaly.ignore", "inventory_anomalies", stringPtrOrNil(anomalyID.String()), map[string]any{"note": body.Note})
