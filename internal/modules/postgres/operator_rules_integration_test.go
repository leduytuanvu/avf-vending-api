...
func TestOperatorSession_HeartbeatRejectedAfterEnd(t *testing.T) {
	...
	_, err = svc.HeartbeatOperatorSession(ctx, uuid.Nil, testfixtures.DevMachineID, sess.ID)
	require.ErrorIs(t, err, domainoperator.ErrSessionNotActive)
}
...
func TestOperatorInsightLists_AttributionsForFlows(t *testing.T) {
	...
	attr, err := svc.ListActionAttributionsForMachine(ctx, uuid.Nil, testfixtures.DevMachineID, 100)
	require.NoError(t, err)
	...
	byTech, err := svc.ListActionAttributionsForTechnician(ctx, uuid.Nil, tid, 200)
	require.NoError(t, err)
	require.NotEmpty(t, byTech)

	tl, err := svc.BuildMachineOperatorTimeline(ctx, uuid.Nil, testfixtures.DevMachineID, 50)
	require.NoError(t, err)
	require.NotEmpty(t, tl)

	authEvents, err := svc.ListAuthEventsForMachine(ctx, uuid.Nil, testfixtures.DevMachineID, 20)
	require.NoError(t, err)
