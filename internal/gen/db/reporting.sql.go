// Code generated by sqlc. DO NOT EDIT.
// versions:
//   sqlc v1.31.1
// source: reporting.sql

package db

import (
	"context"
	"time"

	"github.com/google/uuid"
	"github.com/jackc/pgx/v5/pgtype"
)

const ReportingCashCollectionsCount = `-- name: ReportingCashCollectionsCount :one
SELECT
    COUNT(*)::bigint AS cnt
FROM cash_collections cc
INNER JOIN machines m ON m.id = cc.machine_id
    AND TRUE
WHERE
    cc.collected_at >= $1::timestamptz
    AND cc.collected_at < $2::timestamptz
    AND (
        $3::uuid = '00000000-0000-0000-0000-000000000000'::uuid
        OR m.site_id = $3::uuid)
    AND (
        $4::uuid = '00000000-0000-0000-0000-000000000000'::uuid
        OR cc.machine_id = $4::uuid)
`

type ReportingCashCollectionsCountParams struct {
	Column1 time.Time
	Column2 time.Time
	Column3 uuid.UUID
	Column4 uuid.UUID
}

func (q *Queries) ReportingCashCollectionsCount(ctx context.Context, arg ReportingCashCollectionsCountParams) (int64, error) {
	row := q.db.QueryRow(ctx, ReportingCashCollectionsCount,
		arg.Column1,
		arg.Column2,
		arg.Column3,
		arg.Column4,
	)
	var cnt int64
	err := row.Scan(&cnt)
	return cnt, err
}

const ReportingCashCollectionsForCompany = `-- name: ReportingCashCollectionsForCompany :many
SELECT
    cc.id,
    cc.machine_id,
    m.site_id,
    m.serial_number AS machine_serial_number,
    s.name AS site_name,
    cc.collected_at,
    cc.opened_at,
    cc.closed_at,
    cc.lifecycle_status,
    cc.amount_minor,
    cc.expected_amount_minor,
    cc.variance_amount_minor,
    cc.currency,
    cc.reconciliation_status,
    cc.created_at
FROM
    cash_collections cc
    INNER JOIN machines m ON m.id = cc.machine_id
        AND TRUE
    INNER JOIN sites s ON s.id = m.site_id
WHERE
    cc.collected_at >= $1::timestamptz
    AND cc.collected_at < $2::timestamptz
    AND (
        $3::uuid = '00000000-0000-0000-0000-000000000000'::uuid
        OR m.site_id = $3::uuid)
    AND (
        $4::uuid = '00000000-0000-0000-0000-000000000000'::uuid
        OR cc.machine_id = $4::uuid)
ORDER BY
    cc.collected_at DESC
`

type ReportingCashCollectionsForCompanyParams struct {
	Column1 time.Time
	Column2 time.Time
	Column3 uuid.UUID
	Column4 uuid.UUID
}

type ReportingCashCollectionsForCompanyRow struct {
	ID                   uuid.UUID
	MachineID            uuid.UUID
	SiteID               uuid.UUID
	MachineSerialNumber  string
	SiteName             string
	CollectedAt          time.Time
	OpenedAt             time.Time
	ClosedAt             pgtype.Timestamptz
	LifecycleStatus      string
	AmountMinor          int64
	ExpectedAmountMinor  int64
	VarianceAmountMinor  int64
	Currency             string
	ReconciliationStatus string
	CreatedAt            time.Time
}

func (q *Queries) ReportingCashCollectionsForCompany(ctx context.Context, arg ReportingCashCollectionsForCompanyParams) ([]ReportingCashCollectionsForCompanyRow, error) {
	rows, err := q.db.Query(ctx, ReportingCashCollectionsForCompany,
		arg.Column1,
		arg.Column2,
		arg.Column3,
		arg.Column4,
	)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ReportingCashCollectionsForCompanyRow{}
	for rows.Next() {
		var i ReportingCashCollectionsForCompanyRow
		if err := rows.Scan(
			&i.ID,
			&i.MachineID,
			&i.SiteID,
			&i.MachineSerialNumber,
			&i.SiteName,
			&i.CollectedAt,
			&i.OpenedAt,
			&i.ClosedAt,
			&i.LifecycleStatus,
			&i.AmountMinor,
			&i.ExpectedAmountMinor,
			&i.VarianceAmountMinor,
			&i.Currency,
			&i.ReconciliationStatus,
			&i.CreatedAt,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const ReportingFailedVends = `-- name: ReportingFailedVends :many
SELECT
    vs.id AS vend_session_id,
    vs.order_id,
    vs.machine_id,
    vs.slot_index,
    vs.product_id,
    vs.failure_reason,
    vs.started_at,
    vs.completed_at,
    vs.created_at,
    o.total_minor,
    o.currency,
    o.status AS order_status
FROM vend_sessions vs
INNER JOIN orders o ON o.id = vs.order_id
WHERE
    vs.state = 'failed'
    AND COALESCE(vs.completed_at, vs.created_at) >= $1::timestamptz
    AND COALESCE(vs.completed_at, vs.created_at) < $2::timestamptz
ORDER BY
    COALESCE(vs.completed_at, vs.created_at) DESC
LIMIT $3 OFFSET $4
`

type ReportingFailedVendsParams struct {
	Column1 time.Time
	Column2 time.Time
	Limit   int32
	Offset  int32
}

type ReportingFailedVendsRow struct {
	VendSessionID uuid.UUID
	OrderID       uuid.UUID
	MachineID     uuid.UUID
	SlotIndex     int32
	ProductID     uuid.UUID
	FailureReason pgtype.Text
	StartedAt     pgtype.Timestamptz
	CompletedAt   pgtype.Timestamptz
	CreatedAt     time.Time
	TotalMinor    int64
	Currency      string
	OrderStatus   string
}

func (q *Queries) ReportingFailedVends(ctx context.Context, arg ReportingFailedVendsParams) ([]ReportingFailedVendsRow, error) {
	rows, err := q.db.Query(ctx, ReportingFailedVends,
		arg.Column1,
		arg.Column2,
		arg.Limit,
		arg.Offset,
	)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ReportingFailedVendsRow{}
	for rows.Next() {
		var i ReportingFailedVendsRow
		if err := rows.Scan(
			&i.VendSessionID,
			&i.OrderID,
			&i.MachineID,
			&i.SlotIndex,
			&i.ProductID,
			&i.FailureReason,
			&i.StartedAt,
			&i.CompletedAt,
			&i.CreatedAt,
			&i.TotalMinor,
			&i.Currency,
			&i.OrderStatus,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const ReportingFailedVendsCount = `-- name: ReportingFailedVendsCount :one
SELECT
    COUNT(*)::bigint AS cnt
FROM vend_sessions vs
INNER JOIN orders o ON o.id = vs.order_id
WHERE
    vs.state = 'failed'
    AND COALESCE(vs.completed_at, vs.created_at) >= $1::timestamptz
    AND COALESCE(vs.completed_at, vs.created_at) < $2::timestamptz
`

type ReportingFailedVendsCountParams struct {
	Column1 time.Time
	Column2 time.Time
}

func (q *Queries) ReportingFailedVendsCount(ctx context.Context, arg ReportingFailedVendsCountParams) (int64, error) {
	row := q.db.QueryRow(ctx, ReportingFailedVendsCount, arg.Column1, arg.Column2)
	var cnt int64
	err := row.Scan(&cnt)
	return cnt, err
}

const ReportingFleetIncidentsByStatus = `-- name: ReportingFleetIncidentsByStatus :many
SELECT
    i.status,
    COUNT(*)::bigint AS incident_count
FROM incidents i
WHERE
    i.opened_at >= $1::timestamptz
    AND i.opened_at < $2::timestamptz
GROUP BY
    i.status
ORDER BY
    incident_count DESC
`

type ReportingFleetIncidentsByStatusParams struct {
	Column1 time.Time
	Column2 time.Time
}

type ReportingFleetIncidentsByStatusRow struct {
	Status        string
	IncidentCount int64
}

func (q *Queries) ReportingFleetIncidentsByStatus(ctx context.Context, arg ReportingFleetIncidentsByStatusParams) ([]ReportingFleetIncidentsByStatusRow, error) {
	rows, err := q.db.Query(ctx, ReportingFleetIncidentsByStatus, arg.Column1, arg.Column2)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ReportingFleetIncidentsByStatusRow{}
	for rows.Next() {
		var i ReportingFleetIncidentsByStatusRow
		if err := rows.Scan(&i.Status, &i.IncidentCount); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const ReportingFleetMachineIncidentsBySeverity = `-- name: ReportingFleetMachineIncidentsBySeverity :many
SELECT
    mi.severity,
    COUNT(*)::bigint AS incident_count
FROM machine_incidents mi
INNER JOIN machines m ON m.id = mi.machine_id
WHERE
    mi.opened_at >= $1::timestamptz
    AND mi.opened_at < $2::timestamptz
GROUP BY
    mi.severity
ORDER BY
    incident_count DESC
`

type ReportingFleetMachineIncidentsBySeverityParams struct {
	Column1 time.Time
	Column2 time.Time
}

type ReportingFleetMachineIncidentsBySeverityRow struct {
	Severity      string
	IncidentCount int64
}

func (q *Queries) ReportingFleetMachineIncidentsBySeverity(ctx context.Context, arg ReportingFleetMachineIncidentsBySeverityParams) ([]ReportingFleetMachineIncidentsBySeverityRow, error) {
	rows, err := q.db.Query(ctx, ReportingFleetMachineIncidentsBySeverity, arg.Column1, arg.Column2)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ReportingFleetMachineIncidentsBySeverityRow{}
	for rows.Next() {
		var i ReportingFleetMachineIncidentsBySeverityRow
		if err := rows.Scan(&i.Severity, &i.IncidentCount); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const ReportingFleetMachinesByStatus = `-- name: ReportingFleetMachinesByStatus :many
SELECT
    m.status,
    COUNT(*)::bigint AS machine_count
FROM machines m
GROUP BY
    m.status
ORDER BY
    machine_count DESC
`

type ReportingFleetMachinesByStatusRow struct {
	Status       string
	MachineCount int64
}

func (q *Queries) ReportingFleetMachinesByStatus(ctx context.Context) ([]ReportingFleetMachinesByStatusRow, error) {
	rows, err := q.db.Query(ctx, ReportingFleetMachinesByStatus)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ReportingFleetMachinesByStatusRow{}
	for rows.Next() {
		var i ReportingFleetMachinesByStatusRow
		if err := rows.Scan(&i.Status, &i.MachineCount); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const ReportingInventoryExceptions = `-- name: ReportingInventoryExceptions :many
SELECT
    m.id AS machine_id,
    m.name AS machine_name,
    m.serial_number AS machine_serial_number,
    m.status AS machine_status,
    mss.planogram_id,
    pg.name AS planogram_name,
    mss.slot_index,
    mss.current_quantity,
    COALESCE(s.max_quantity, 0)::int AS max_quantity,
    s.product_id,
    pr.name AS product_name,
    pr.sku AS product_sku,
    (mss.current_quantity <= 0) AS out_of_stock,
    (
        COALESCE(s.max_quantity, 0) > 0
        AND mss.current_quantity > 0
        AND mss.current_quantity::float / NULLIF(s.max_quantity, 0)::float < 0.15
    ) AS low_stock
FROM machine_slot_state mss
INNER JOIN machines m ON m.id = mss.machine_id
INNER JOIN planograms pg ON pg.id = mss.planogram_id
LEFT JOIN slots s ON s.planogram_id = mss.planogram_id
    AND s.slot_index = mss.slot_index
LEFT JOIN products pr ON pr.id = s.product_id
WHERE
    (
        (
            $1::boolean IS TRUE
            AND mss.current_quantity <= 0
        )
        OR (
            $2::boolean IS TRUE
            AND COALESCE(s.max_quantity, 0) > 0
            AND mss.current_quantity > 0
            AND mss.current_quantity::float / NULLIF(s.max_quantity, 0)::float < 0.15
        )
    )
ORDER BY
    m.name ASC,
    mss.slot_index ASC
LIMIT $3 OFFSET $4
`

type ReportingInventoryExceptionsParams struct {
	Column1 bool
	Column2 bool
	Limit   int32
	Offset  int32
}

type ReportingInventoryExceptionsRow struct {
	MachineID           uuid.UUID
	MachineName         string
	MachineSerialNumber string
	MachineStatus       string
	PlanogramID         uuid.UUID
	PlanogramName       string
	SlotIndex           int32
	CurrentQuantity     int32
	MaxQuantity         int32
	ProductID           pgtype.UUID
	ProductName         pgtype.Text
	ProductSku          pgtype.Text
	OutOfStock          bool
	LowStock            pgtype.Bool
}

func (q *Queries) ReportingInventoryExceptions(ctx context.Context, arg ReportingInventoryExceptionsParams) ([]ReportingInventoryExceptionsRow, error) {
	rows, err := q.db.Query(ctx, ReportingInventoryExceptions,
		arg.Column1,
		arg.Column2,
		arg.Limit,
		arg.Offset,
	)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ReportingInventoryExceptionsRow{}
	for rows.Next() {
		var i ReportingInventoryExceptionsRow
		if err := rows.Scan(
			&i.MachineID,
			&i.MachineName,
			&i.MachineSerialNumber,
			&i.MachineStatus,
			&i.PlanogramID,
			&i.PlanogramName,
			&i.SlotIndex,
			&i.CurrentQuantity,
			&i.MaxQuantity,
			&i.ProductID,
			&i.ProductName,
			&i.ProductSku,
			&i.OutOfStock,
			&i.LowStock,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const ReportingInventoryExceptionsCount = `-- name: ReportingInventoryExceptionsCount :one
SELECT
    count(*)::bigint AS cnt
FROM machine_slot_state mss
INNER JOIN machines m ON m.id = mss.machine_id
LEFT JOIN slots s ON s.planogram_id = mss.planogram_id
    AND s.slot_index = mss.slot_index
WHERE
    (
        (
            $1::boolean IS TRUE
            AND mss.current_quantity <= 0
        )
        OR (
            $2::boolean IS TRUE
            AND COALESCE(s.max_quantity, 0) > 0
            AND mss.current_quantity > 0
            AND mss.current_quantity::float / NULLIF(s.max_quantity, 0)::float < 0.15
        )
    )
`

type ReportingInventoryExceptionsCountParams struct {
	Column1 bool
	Column2 bool
}

func (q *Queries) ReportingInventoryExceptionsCount(ctx context.Context, arg ReportingInventoryExceptionsCountParams) (int64, error) {
	row := q.db.QueryRow(ctx, ReportingInventoryExceptionsCount, arg.Column1, arg.Column2)
	var cnt int64
	err := row.Scan(&cnt)
	return cnt, err
}

const ReportingMachineHealth = `-- name: ReportingMachineHealth :many
SELECT
    m.id AS machine_id,
    m.site_id,
    s.name AS site_name,
    m.serial_number,
    m.name AS machine_name,
    m.status,
    m.last_seen_at,
    CASE
        WHEN m.status IN ('offline', 'suspended', 'maintenance', 'retired', 'decommissioned', 'compromised') THEN TRUE
        WHEN m.last_seen_at IS NULL THEN TRUE
        WHEN m.last_seen_at < $1::timestamptz THEN TRUE
        ELSE FALSE
    END AS offline
FROM machines m
INNER JOIN sites s ON s.id = m.site_id
ORDER BY
    offline DESC,
    m.last_seen_at ASC NULLS FIRST,
    m.name ASC
LIMIT $2 OFFSET $3
`

type ReportingMachineHealthParams struct {
	Column1 time.Time
	Limit   int32
	Offset  int32
}

type ReportingMachineHealthRow struct {
	MachineID    uuid.UUID
	SiteID       uuid.UUID
	SiteName     string
	SerialNumber string
	MachineName  string
	Status       string
	LastSeenAt   pgtype.Timestamptz
	Offline      bool
}

func (q *Queries) ReportingMachineHealth(ctx context.Context, arg ReportingMachineHealthParams) ([]ReportingMachineHealthRow, error) {
	rows, err := q.db.Query(ctx, ReportingMachineHealth, arg.Column1, arg.Limit, arg.Offset)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ReportingMachineHealthRow{}
	for rows.Next() {
		var i ReportingMachineHealthRow
		if err := rows.Scan(
			&i.MachineID,
			&i.SiteID,
			&i.SiteName,
			&i.SerialNumber,
			&i.MachineName,
			&i.Status,
			&i.LastSeenAt,
			&i.Offline,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const ReportingMachineHealthCount = `-- name: ReportingMachineHealthCount :one
SELECT
    COUNT(*)::bigint AS cnt
FROM machines m
`

func (q *Queries) ReportingMachineHealthCount(ctx context.Context) (int64, error) {
	row := q.db.QueryRow(ctx, ReportingMachineHealthCount)
	var cnt int64
	err := row.Scan(&cnt)
	return cnt, err
}

const ReportingPaymentSettlement = `-- name: ReportingPaymentSettlement :many
SELECT
    (date_trunc('day', p.created_at AT TIME ZONE $1::text) AT TIME ZONE $1::text)::timestamptz AS bucket_start,
    p.provider,
    p.state,
    p.settlement_status,
    p.reconciliation_status,
    COUNT(*)::bigint AS payment_count,
    COALESCE(SUM(p.amount_minor), 0)::bigint AS amount_minor
FROM payments p
INNER JOIN orders o ON o.id = p.order_id
WHERE
    p.created_at >= $2::timestamptz
    AND p.created_at < $3::timestamptz
GROUP BY
    1,
    p.provider,
    p.state,
    p.settlement_status,
    p.reconciliation_status
ORDER BY
    1 ASC,
    p.provider ASC,
    p.settlement_status ASC,
    p.state ASC
`

type ReportingPaymentSettlementParams struct {
	Column1 string
	Column2 time.Time
	Column3 time.Time
}

type ReportingPaymentSettlementRow struct {
	BucketStart          time.Time
	Provider             string
	State                string
	SettlementStatus     string
	ReconciliationStatus string
	PaymentCount         int64
	AmountMinor          int64
}

func (q *Queries) ReportingPaymentSettlement(ctx context.Context, arg ReportingPaymentSettlementParams) ([]ReportingPaymentSettlementRow, error) {
	rows, err := q.db.Query(ctx, ReportingPaymentSettlement, arg.Column1, arg.Column2, arg.Column3)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ReportingPaymentSettlementRow{}
	for rows.Next() {
		var i ReportingPaymentSettlementRow
		if err := rows.Scan(
			&i.BucketStart,
			&i.Provider,
			&i.State,
			&i.SettlementStatus,
			&i.ReconciliationStatus,
			&i.PaymentCount,
			&i.AmountMinor,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const ReportingPaymentsByDay = `-- name: ReportingPaymentsByDay :many
SELECT
    (date_trunc('day', p.created_at AT TIME ZONE $1::text) AT TIME ZONE $1::text)::timestamptz AS bucket_start,
    COUNT(*)::bigint AS payment_count,
    COALESCE(SUM(p.amount_minor), 0)::bigint AS amount_minor
FROM payments p
INNER JOIN orders o ON o.id = p.order_id
WHERE
    p.created_at >= $2::timestamptz
    AND p.created_at < $3::timestamptz
GROUP BY
    1
ORDER BY
    1 ASC
`

type ReportingPaymentsByDayParams struct {
	Column1 string
	Column2 time.Time
	Column3 time.Time
}

type ReportingPaymentsByDayRow struct {
	BucketStart  time.Time
	PaymentCount int64
	AmountMinor  int64
}

func (q *Queries) ReportingPaymentsByDay(ctx context.Context, arg ReportingPaymentsByDayParams) ([]ReportingPaymentsByDayRow, error) {
	rows, err := q.db.Query(ctx, ReportingPaymentsByDay, arg.Column1, arg.Column2, arg.Column3)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ReportingPaymentsByDayRow{}
	for rows.Next() {
		var i ReportingPaymentsByDayRow
		if err := rows.Scan(&i.BucketStart, &i.PaymentCount, &i.AmountMinor); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const ReportingPaymentsByMethodAndState = `-- name: ReportingPaymentsByMethodAndState :many
SELECT
    p.provider,
    p.state,
    COUNT(*)::bigint AS payment_count,
    COALESCE(SUM(p.amount_minor), 0)::bigint AS amount_minor
FROM payments p
INNER JOIN orders o ON o.id = p.order_id
WHERE
    p.created_at >= $1::timestamptz
    AND p.created_at < $2::timestamptz
GROUP BY
    p.provider,
    p.state
ORDER BY
    amount_minor DESC
`

type ReportingPaymentsByMethodAndStateParams struct {
	Column1 time.Time
	Column2 time.Time
}

type ReportingPaymentsByMethodAndStateRow struct {
	Provider     string
	State        string
	PaymentCount int64
	AmountMinor  int64
}

func (q *Queries) ReportingPaymentsByMethodAndState(ctx context.Context, arg ReportingPaymentsByMethodAndStateParams) ([]ReportingPaymentsByMethodAndStateRow, error) {
	rows, err := q.db.Query(ctx, ReportingPaymentsByMethodAndState, arg.Column1, arg.Column2)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ReportingPaymentsByMethodAndStateRow{}
	for rows.Next() {
		var i ReportingPaymentsByMethodAndStateRow
		if err := rows.Scan(
			&i.Provider,
			&i.State,
			&i.PaymentCount,
			&i.AmountMinor,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const ReportingPaymentsTotals = `-- name: ReportingPaymentsTotals :one
SELECT
    COUNT(*) FILTER (
        WHERE
            p.state = 'authorized'
    )::bigint AS authorized_count,
    COUNT(*) FILTER (
        WHERE
            p.state = 'captured'
    )::bigint AS captured_count,
    COUNT(*) FILTER (
        WHERE
            p.state = 'failed'
    )::bigint AS failed_count,
    COUNT(*) FILTER (
        WHERE
            p.state = 'refunded'
    )::bigint AS refunded_count,
    COALESCE(SUM(p.amount_minor) FILTER (
        WHERE
            p.state = 'captured'
    ), 0)::bigint AS captured_amount_minor,
    COALESCE(SUM(p.amount_minor) FILTER (
        WHERE
            p.state = 'authorized'
    ), 0)::bigint AS authorized_amount_minor,
    COALESCE(SUM(p.amount_minor) FILTER (
        WHERE
            p.state = 'failed'
    ), 0)::bigint AS failed_amount_minor,
    COALESCE(SUM(p.amount_minor) FILTER (
        WHERE
            p.state = 'refunded'
    ), 0)::bigint AS refunded_amount_minor
FROM payments p
INNER JOIN orders o ON o.id = p.order_id
WHERE
    p.created_at >= $1::timestamptz
    AND p.created_at < $2::timestamptz
`

type ReportingPaymentsTotalsParams struct {
	Column1 time.Time
	Column2 time.Time
}

type ReportingPaymentsTotalsRow struct {
	AuthorizedCount       int64
	CapturedCount         int64
	FailedCount           int64
	RefundedCount         int64
	CapturedAmountMinor   int64
	AuthorizedAmountMinor int64
	FailedAmountMinor     int64
	RefundedAmountMinor   int64
}

func (q *Queries) ReportingPaymentsTotals(ctx context.Context, arg ReportingPaymentsTotalsParams) (ReportingPaymentsTotalsRow, error) {
	row := q.db.QueryRow(ctx, ReportingPaymentsTotals, arg.Column1, arg.Column2)
	var i ReportingPaymentsTotalsRow
	err := row.Scan(
		&i.AuthorizedCount,
		&i.CapturedCount,
		&i.FailedCount,
		&i.RefundedCount,
		&i.CapturedAmountMinor,
		&i.AuthorizedAmountMinor,
		&i.FailedAmountMinor,
		&i.RefundedAmountMinor,
	)
	return i, err
}

const ReportingReconciliationQueue = `-- name: ReportingReconciliationQueue :many
SELECT
    c.id,
    c.case_type,
    c.status,
    c.severity,
    c.order_id,
    c.payment_id,
    c.vend_session_id,
    c.refund_id,
    c.provider,
    c.reason,
    c.first_detected_at,
    c.last_detected_at
FROM commerce_reconciliation_cases c
WHERE
    c.first_detected_at >= $1::timestamptz
    AND c.first_detected_at < $2::timestamptz
    AND c.status IN ('open', 'reviewing')
ORDER BY
    c.severity DESC,
    c.last_detected_at DESC
LIMIT $3 OFFSET $4
`

type ReportingReconciliationQueueParams struct {
	Column1 time.Time
	Column2 time.Time
	Limit   int32
	Offset  int32
}

type ReportingReconciliationQueueRow struct {
	ID              uuid.UUID
	CaseType        string
	Status          string
	Severity        string
	OrderID         pgtype.UUID
	PaymentID       pgtype.UUID
	VendSessionID   pgtype.UUID
	RefundID        pgtype.UUID
	Provider        pgtype.Text
	Reason          string
	FirstDetectedAt time.Time
	LastDetectedAt  time.Time
}

func (q *Queries) ReportingReconciliationQueue(ctx context.Context, arg ReportingReconciliationQueueParams) ([]ReportingReconciliationQueueRow, error) {
	rows, err := q.db.Query(ctx, ReportingReconciliationQueue,
		arg.Column1,
		arg.Column2,
		arg.Limit,
		arg.Offset,
	)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ReportingReconciliationQueueRow{}
	for rows.Next() {
		var i ReportingReconciliationQueueRow
		if err := rows.Scan(
			&i.ID,
			&i.CaseType,
			&i.Status,
			&i.Severity,
			&i.OrderID,
			&i.PaymentID,
			&i.VendSessionID,
			&i.RefundID,
			&i.Provider,
			&i.Reason,
			&i.FirstDetectedAt,
			&i.LastDetectedAt,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const ReportingReconciliationQueueCount = `-- name: ReportingReconciliationQueueCount :one
SELECT
    COUNT(*)::bigint AS cnt
FROM commerce_reconciliation_cases c
WHERE
    c.first_detected_at >= $1::timestamptz
    AND c.first_detected_at < $2::timestamptz
    AND c.status IN ('open', 'reviewing')
`

type ReportingReconciliationQueueCountParams struct {
	Column1 time.Time
	Column2 time.Time
}

func (q *Queries) ReportingReconciliationQueueCount(ctx context.Context, arg ReportingReconciliationQueueCountParams) (int64, error) {
	row := q.db.QueryRow(ctx, ReportingReconciliationQueueCount, arg.Column1, arg.Column2)
	var cnt int64
	err := row.Scan(&cnt)
	return cnt, err
}

const ReportingRefunds = `-- name: ReportingRefunds :many
SELECT
    r.id AS refund_id,
    r.payment_id,
    r.order_id,
    o.machine_id,
    r.amount_minor,
    r.currency,
    r.state,
    r.reason,
    r.reconciliation_status,
    r.settlement_status,
    r.created_at
FROM refunds r
INNER JOIN orders o ON o.id = r.order_id
WHERE
    r.created_at >= $1::timestamptz
    AND r.created_at < $2::timestamptz
ORDER BY
    r.created_at DESC
LIMIT $3 OFFSET $4
`

type ReportingRefundsParams struct {
	Column1 time.Time
	Column2 time.Time
	Limit   int32
	Offset  int32
}

type ReportingRefundsRow struct {
	RefundID             uuid.UUID
	PaymentID            uuid.UUID
	OrderID              uuid.UUID
	MachineID            uuid.UUID
	AmountMinor          int64
	Currency             string
	State                string
	Reason               pgtype.Text
	ReconciliationStatus string
	SettlementStatus     string
	CreatedAt            time.Time
}

func (q *Queries) ReportingRefunds(ctx context.Context, arg ReportingRefundsParams) ([]ReportingRefundsRow, error) {
	rows, err := q.db.Query(ctx, ReportingRefunds,
		arg.Column1,
		arg.Column2,
		arg.Limit,
		arg.Offset,
	)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ReportingRefundsRow{}
	for rows.Next() {
		var i ReportingRefundsRow
		if err := rows.Scan(
			&i.RefundID,
			&i.PaymentID,
			&i.OrderID,
			&i.MachineID,
			&i.AmountMinor,
			&i.Currency,
			&i.State,
			&i.Reason,
			&i.ReconciliationStatus,
			&i.SettlementStatus,
			&i.CreatedAt,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const ReportingRefundsCount = `-- name: ReportingRefundsCount :one
SELECT
    COUNT(*)::bigint AS cnt
FROM refunds r
INNER JOIN orders o ON o.id = r.order_id
WHERE
    r.created_at >= $1::timestamptz
    AND r.created_at < $2::timestamptz
`

type ReportingRefundsCountParams struct {
	Column1 time.Time
	Column2 time.Time
}

func (q *Queries) ReportingRefundsCount(ctx context.Context, arg ReportingRefundsCountParams) (int64, error) {
	row := q.db.QueryRow(ctx, ReportingRefundsCount, arg.Column1, arg.Column2)
	var cnt int64
	err := row.Scan(&cnt)
	return cnt, err
}

const ReportingSalesByDay = `-- name: ReportingSalesByDay :many
SELECT
    (date_trunc('day', o.created_at AT TIME ZONE $1::text) AT TIME ZONE $1::text)::timestamptz AS bucket_start,
    COUNT(*)::bigint AS order_count,
    COALESCE(SUM(o.total_minor), 0)::bigint AS total_minor,
    COALESCE(SUM(o.subtotal_minor), 0)::bigint AS subtotal_minor,
    COALESCE(SUM(o.tax_minor), 0)::bigint AS tax_minor
FROM orders o
WHERE
    o.created_at >= $2::timestamptz
    AND o.created_at < $3::timestamptz
GROUP BY
    1
ORDER BY
    1 ASC
`

type ReportingSalesByDayParams struct {
	Column1 string
	Column2 time.Time
	Column3 time.Time
}

type ReportingSalesByDayRow struct {
	BucketStart   time.Time
	OrderCount    int64
	TotalMinor    int64
	SubtotalMinor int64
	TaxMinor      int64
}

func (q *Queries) ReportingSalesByDay(ctx context.Context, arg ReportingSalesByDayParams) ([]ReportingSalesByDayRow, error) {
	rows, err := q.db.Query(ctx, ReportingSalesByDay, arg.Column1, arg.Column2, arg.Column3)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ReportingSalesByDayRow{}
	for rows.Next() {
		var i ReportingSalesByDayRow
		if err := rows.Scan(
			&i.BucketStart,
			&i.OrderCount,
			&i.TotalMinor,
			&i.SubtotalMinor,
			&i.TaxMinor,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const ReportingSalesByMachine = `-- name: ReportingSalesByMachine :many
SELECT
    o.machine_id,
    COUNT(*)::bigint AS order_count,
    COALESCE(SUM(o.total_minor), 0)::bigint AS total_minor,
    COALESCE(SUM(o.subtotal_minor), 0)::bigint AS subtotal_minor,
    COALESCE(SUM(o.tax_minor), 0)::bigint AS tax_minor
FROM orders o
WHERE
    o.created_at >= $1::timestamptz
    AND o.created_at < $2::timestamptz
GROUP BY
    o.machine_id
ORDER BY
    total_minor DESC
LIMIT 500
`

type ReportingSalesByMachineParams struct {
	Column1 time.Time
	Column2 time.Time
}

type ReportingSalesByMachineRow struct {
	MachineID     uuid.UUID
	OrderCount    int64
	TotalMinor    int64
	SubtotalMinor int64
	TaxMinor      int64
}

func (q *Queries) ReportingSalesByMachine(ctx context.Context, arg ReportingSalesByMachineParams) ([]ReportingSalesByMachineRow, error) {
	rows, err := q.db.Query(ctx, ReportingSalesByMachine, arg.Column1, arg.Column2)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ReportingSalesByMachineRow{}
	for rows.Next() {
		var i ReportingSalesByMachineRow
		if err := rows.Scan(
			&i.MachineID,
			&i.OrderCount,
			&i.TotalMinor,
			&i.SubtotalMinor,
			&i.TaxMinor,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const ReportingSalesByPaymentProvider = `-- name: ReportingSalesByPaymentProvider :many
SELECT
    p.provider,
    COUNT(DISTINCT o.id)::bigint AS order_count,
    COALESCE(SUM(o.total_minor), 0)::bigint AS total_minor,
    COALESCE(SUM(o.subtotal_minor), 0)::bigint AS subtotal_minor,
    COALESCE(SUM(o.tax_minor), 0)::bigint AS tax_minor
FROM orders o
INNER JOIN payments p ON p.order_id = o.id
    AND p.state IN ('authorized', 'captured')
WHERE
    o.created_at >= $1::timestamptz
    AND o.created_at < $2::timestamptz
GROUP BY
    p.provider
ORDER BY
    total_minor DESC
`

type ReportingSalesByPaymentProviderParams struct {
	Column1 time.Time
	Column2 time.Time
}

type ReportingSalesByPaymentProviderRow struct {
	Provider      string
	OrderCount    int64
	TotalMinor    int64
	SubtotalMinor int64
	TaxMinor      int64
}

func (q *Queries) ReportingSalesByPaymentProvider(ctx context.Context, arg ReportingSalesByPaymentProviderParams) ([]ReportingSalesByPaymentProviderRow, error) {
	rows, err := q.db.Query(ctx, ReportingSalesByPaymentProvider, arg.Column1, arg.Column2)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ReportingSalesByPaymentProviderRow{}
	for rows.Next() {
		var i ReportingSalesByPaymentProviderRow
		if err := rows.Scan(
			&i.Provider,
			&i.OrderCount,
			&i.TotalMinor,
			&i.SubtotalMinor,
			&i.TaxMinor,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const ReportingSalesBySite = `-- name: ReportingSalesBySite :many
SELECT
    m.site_id,
    COUNT(*)::bigint AS order_count,
    COALESCE(SUM(o.total_minor), 0)::bigint AS total_minor,
    COALESCE(SUM(o.subtotal_minor), 0)::bigint AS subtotal_minor,
    COALESCE(SUM(o.tax_minor), 0)::bigint AS tax_minor
FROM orders o
INNER JOIN machines m ON m.id = o.machine_id
WHERE
    o.created_at >= $1::timestamptz
    AND o.created_at < $2::timestamptz
GROUP BY
    m.site_id
ORDER BY
    total_minor DESC
`

type ReportingSalesBySiteParams struct {
	Column1 time.Time
	Column2 time.Time
}

type ReportingSalesBySiteRow struct {
	SiteID        uuid.UUID
	OrderCount    int64
	TotalMinor    int64
	SubtotalMinor int64
	TaxMinor      int64
}

func (q *Queries) ReportingSalesBySite(ctx context.Context, arg ReportingSalesBySiteParams) ([]ReportingSalesBySiteRow, error) {
	rows, err := q.db.Query(ctx, ReportingSalesBySite, arg.Column1, arg.Column2)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ReportingSalesBySiteRow{}
	for rows.Next() {
		var i ReportingSalesBySiteRow
		if err := rows.Scan(
			&i.SiteID,
			&i.OrderCount,
			&i.TotalMinor,
			&i.SubtotalMinor,
			&i.TaxMinor,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const ReportingSalesTotals = `-- name: ReportingSalesTotals :one
SELECT
    COALESCE(SUM(o.total_minor), 0)::bigint AS gross_total_minor,
    COALESCE(SUM(o.tax_minor), 0)::bigint AS tax_minor,
    COALESCE(SUM(o.subtotal_minor), 0)::bigint AS subtotal_minor,
    COUNT(*)::bigint AS order_count
FROM orders o
WHERE
    o.created_at >= $1::timestamptz
    AND o.created_at < $2::timestamptz
`

type ReportingSalesTotalsParams struct {
	Column1 time.Time
	Column2 time.Time
}

type ReportingSalesTotalsRow struct {
	GrossTotalMinor int64
	TaxMinor        int64
	SubtotalMinor   int64
	OrderCount      int64
}

func (q *Queries) ReportingSalesTotals(ctx context.Context, arg ReportingSalesTotalsParams) (ReportingSalesTotalsRow, error) {
	row := q.db.QueryRow(ctx, ReportingSalesTotals, arg.Column1, arg.Column2)
	var i ReportingSalesTotalsRow
	err := row.Scan(
		&i.GrossTotalMinor,
		&i.TaxMinor,
		&i.SubtotalMinor,
		&i.OrderCount,
	)
	return i, err
}
