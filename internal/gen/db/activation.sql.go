...
type ListMachineActivationCodesForMachineRow struct {
	ID                     uuid.UUID
	MachineID              uuid.UUID
	CodeHash               []byte
	MaxUses                int32
...
func (q *Queries) ListMachineActivationCodesForMachine(ctx context.Context, machineID uuid.UUID) ([]ListMachineActivationCodesForMachineRow, error) {
	rows, err := q.db.Query(ctx, ListMachineActivationCodesForMachine, machineID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ListMachineActivationCodesForMachineRow{}
	for rows.Next() {
		var i ListMachineActivationCodesForMachineRow
		if err := rows.Scan(
			&i.ID,
			&i.MachineID,
			&i.CodeHash,
...
type ListMachineActivationCodesPagedRow struct {
	ID                     uuid.UUID
	MachineID              uuid.UUID
	CodeHash               []byte
	MaxUses                int32
...
func (q *Queries) ListMachineActivationCodesPaged(ctx context.Context, arg ListMachineActivationCodesPagedParams) ([]ListMachineActivationCodesPagedRow, error) {
	rows, err := q.db.Query(ctx, ListMachineActivationCodesPaged, arg.Limit, arg.Offset)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ListMachineActivationCodesPagedRow{}
	for rows.Next() {
		var i ListMachineActivationCodesPagedRow
		if err := rows.Scan(
			&i.ID,
			&i.MachineID,
			&i.CodeHash,
