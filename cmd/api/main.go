...
// @description **OpenAPI 3.0.3** JSON: `GET /swagger/doc.json` when `HTTP_OPENAPI_JSON_ENABLED=true`. In **production** this defaults to **off** when unset and additionally requires `PRODUCTION_OPENAPI_JSON_ALLOWED=true` when explicitly enabled. **Swagger UI** (`/swagger/`, `index.html`) is separate, controlled by `HTTP_SWAGGER_UI_ENABLED` (default **on** in dev/staging, **off** in production) and in production requires `PRODUCTION_SWAGGER_UI_ALLOWED=true` when explicitly enabled—use JSON-only for Postman imports without exposing the browser UI. Regenerate the embedded spec with `make swagger` or `tools/build_openapi.py` when routes change.
// @description **Errors:** JSON responses share one envelope: `{"error":{"code","message","details","requestId"}}`. `details` is always an object (empty `{}` when there is nothing extra). Middleware and handlers use the same shape. Typical codes: `unauthenticated`, `forbidden`, `company_scope_required`, `invalid_machine_id`, `auth_misconfigured` (JWT validation misconfiguration, HTTP 503), `rate_limited` (429).
...
// @description **Artifacts:** When `API_ARTIFACTS_ENABLED=true`, `POST /v1/admin/artifacts` reserves an id + upload path, then clients `PUT` bytes to object storage using returned instructions—see Artifacts tag routes.
