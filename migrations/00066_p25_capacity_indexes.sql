...
CREATE INDEX IF NOT EXISTS ix_orders_company_machine_created_at ON orders (scope_id, machine_id, created_at DESC);
...
CREATE INDEX IF NOT EXISTS ix_audit_events_company_occurred_action ON audit_events (scope_id, occurred_at DESC, action);
...
COMMENT ON INDEX ix_orders_company_machine_created_at IS 'P2.5: company + machine reporting windows on orders.created_at.';
...
COMMENT ON INDEX ix_audit_events_company_occurred_action IS 'P2.5: company audit timelines by occurred_at.';
...
DROP INDEX IF EXISTS ix_orders_company_machine_created_at;
