...
CREATE INDEX ix_audit_events_org_created ON audit_events (scope_id, created_at DESC);

CREATE INDEX ix_audit_events_org_action ON audit_events (scope_id, action);
...
CREATE INDEX ix_audit_events_org_resource ON audit_events (scope_id, resource_type, resource_id)
WHERE
