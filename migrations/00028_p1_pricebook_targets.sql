...
-- Assignment of company-scoped price books to specific machines or sites (many-to-many).
CREATE TABLE price_book_targets (
    ...
    CONSTRAINT fk_price_book_targets_org_book FOREIGN KEY (scope_id, price_book_id)
        REFERENCES price_books (scope_id, id) ON DELETE CASCADE,
    CONSTRAINT fk_price_book_targets_org_site FOREIGN KEY (scope_id, site_id)
        REFERENCES sites (scope_id, id),
    CONSTRAINT fk_price_book_targets_org_machine FOREIGN KEY (scope_id, machine_id)
...
CREATE INDEX ix_price_book_targets_scope_id ON price_book_targets (scope_id);
CREATE INDEX ix_price_book_targets_book ON price_book_targets (scope_id, price_book_id);

COMMENT ON TABLE price_book_targets IS 'Links company-scoped price books to a machine or site for layered pricing; precedence vs scoped rows is resolved at preview time.';
