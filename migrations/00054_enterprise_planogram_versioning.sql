...
CREATE INDEX ix_planogram_templates_org ON planogram_templates (scope_id, created_at DESC);
...
CREATE TABLE machine_planogram_drafts (
    ...
    created_at timestamptz NOT NULL DEFAULT now (),
    updated_at timestamptz NOT NULL DEFAULT now (),
    CONSTRAINT fk_machine_planogram_drafts_org_machine FOREIGN KEY (scope_id, machine_id) REFERENCES machines (scope_id, id) ON DELETE CASCADE
);
...
CREATE TABLE machine_planogram_versions (
    ...
    CONSTRAINT fk_machine_planogram_versions_org_machine FOREIGN KEY (scope_id, machine_id) REFERENCES machines (scope_id, id) ON DELETE CASCADE
);
