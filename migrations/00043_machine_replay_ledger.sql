...
CREATE TABLE IF NOT EXISTS machine_idempotency_keys (
    ...
    CONSTRAINT ux_machine_idempotency_key UNIQUE (scope_id, machine_id, operation, idempotency_key)
);
...
CREATE TABLE IF NOT EXISTS machine_offline_events (
    ...
    CONSTRAINT ux_machine_offline_sequence UNIQUE (scope_id, machine_id, offline_sequence)
);

CREATE INDEX IF NOT EXISTS ix_machine_offline_pending ON machine_offline_events (scope_id, machine_id, offline_sequence)
WHERE processing_status IN ('pending', 'processing');

CREATE INDEX IF NOT EXISTS ix_machine_offline_event_id ON machine_offline_events (scope_id, machine_id, event_id)
WHERE event_id <> '';
...
CREATE TABLE IF NOT EXISTS machine_sync_cursors (
    ...
    PRIMARY KEY (scope_id, machine_id, stream_name)
);
