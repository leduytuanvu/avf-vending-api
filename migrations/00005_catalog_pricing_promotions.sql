...
-- ---------------------------------------------------------------------------
-- Categories & brands (role-scoped catalog)
-- ---------------------------------------------------------------------------
...
CREATE UNIQUE INDEX ux_categories_org_slug_lower ON categories (scope_id, lower(slug));
CREATE UNIQUE INDEX ux_categories_scope_pk ON categories (scope_id, id);

CREATE INDEX ix_categories_scope_id ON categories (scope_id);
CREATE INDEX ix_categories_parent_id ON categories (parent_id);

COMMENT ON TABLE categories IS 'Product taxonomy per company; slug unique per org (case-insensitive).';
...
CREATE UNIQUE INDEX ux_brands_org_slug_lower ON brands (scope_id, lower(slug));
CREATE UNIQUE INDEX ux_brands_scope_pk ON brands (scope_id, id);

CREATE INDEX ix_brands_scope_id ON brands (scope_id);

COMMENT ON TABLE brands IS 'Manufacturer / brand labels scoped to company.';
...
ALTER TABLE products
    ADD CONSTRAINT fk_products_org_category FOREIGN KEY (scope_id, category_id)
        REFERENCES categories (scope_id, id),
    ADD CONSTRAINT fk_products_org_brand FOREIGN KEY (scope_id, brand_id)
        REFERENCES brands (scope_id, id),
...
CREATE UNIQUE INDEX ux_products_scope_pk ON products (scope_id, id);
...
CREATE UNIQUE INDEX ux_sites_scope_pk ON sites (scope_id, id);
CREATE UNIQUE INDEX ux_machines_scope_pk ON machines (scope_id, id);

ALTER TABLE price_books
    ADD COLUMN scope_type text NOT NULL DEFAULT 'global' CHECK (scope_type IN ('global', 'site', 'machine')),
...
COMMENT ON COLUMN price_books.scope_type IS 'company = default org book; site = location-specific; machine = device override book.';
COMMENT ON COLUMN price_books.priority IS 'Higher wins within same scope level when multiple books match.';

ALTER TABLE price_books
    ADD CONSTRAINT fk_price_books_org_site FOREIGN KEY (scope_id, site_id)
        REFERENCES sites (scope_id, id),
    ADD CONSTRAINT fk_price_books_org_machine FOREIGN KEY (scope_id, machine_id)
        REFERENCES machines (scope_id, id);

CREATE UNIQUE INDEX ux_price_books_scope_pk ON price_books (scope_id, id);

CREATE UNIQUE INDEX ux_price_books_org_scope_org_name_effective
    ON price_books (scope_id, lower(name), effective_from)
    WHERE scope_type = 'global';

CREATE UNIQUE INDEX ux_price_books_org_scope_site_name_effective
    ON price_books (scope_id, site_id, lower(name), effective_from)
    WHERE scope_type = 'site';

CREATE UNIQUE INDEX ux_price_books_org_scope_machine_name_effective
    ON price_books (scope_id, machine_id, lower(name), effective_from)
    WHERE scope_type = 'machine';

-- ---------------------------------------------------------------------------
-- Price book items: company-safe link to book + product
-- ---------------------------------------------------------------------------
...
ALTER TABLE price_book_items
    ADD CONSTRAINT fk_price_book_items_org_book FOREIGN KEY (scope_id, price_book_id)
        REFERENCES price_books (scope_id, id) ON DELETE CASCADE,
    ADD CONSTRAINT fk_price_book_items_org_product FOREIGN KEY (scope_id, product_id)
        REFERENCES products (scope_id, id) ON DELETE RESTRICT;

ALTER TABLE price_book_items DROP CONSTRAINT ux_price_book_items_book_product;

ALTER TABLE price_book_items
    ADD CONSTRAINT ux_price_book_items_org_book_product UNIQUE (scope_id, price_book_id, product_id);

CREATE INDEX ix_price_book_items_scope_id ON price_book_items (scope_id);
...
CREATE TABLE machine_price_overrides (
    ...
    created_at timestamptz NOT NULL DEFAULT now(),
    CONSTRAINT fk_machine_price_overrides_org_machine FOREIGN KEY (scope_id, machine_id)
        REFERENCES machines (scope_id, id),
    CONSTRAINT fk_machine_price_overrides_org_product FOREIGN KEY (scope_id, product_id)
        REFERENCES products (scope_id, id)
);
...
CREATE INDEX ix_machine_price_overrides_scope_id ON machine_price_overrides (scope_id);
...
CREATE UNIQUE INDEX ux_promotions_scope_pk ON promotions (scope_id, id);

CREATE INDEX ix_promotions_scope_id ON promotions (scope_id);
CREATE INDEX ix_promotions_org_approval ON promotions (scope_id, approval_status);
...
CREATE TABLE promotion_targets (
    ...
    site_id uuid,
    created_at timestamptz NOT NULL DEFAULT now(),
    CONSTRAINT fk_promotion_targets_org_promo FOREIGN KEY (scope_id, promotion_id)
        REFERENCES promotions (scope_id, id) ON DELETE CASCADE,
    CONSTRAINT fk_pt_org_product FOREIGN KEY (scope_id, product_id)
        REFERENCES products (scope_id, id) ON DELETE CASCADE,
    CONSTRAINT fk_pt_org_category FOREIGN KEY (scope_id, category_id)
        REFERENCES categories (scope_id, id) ON DELETE CASCADE,
    CONSTRAINT fk_pt_org_machine FOREIGN KEY (scope_id, machine_id)
        REFERENCES machines (scope_id, id) ON DELETE CASCADE,
    CONSTRAINT fk_pt_org_site FOREIGN KEY (scope_id, site_id)
        REFERENCES sites (scope_id, id) ON DELETE CASCADE,
...
CREATE INDEX ix_promotion_targets_scope_id ON promotion_targets (scope_id);
...
DROP INDEX IF EXISTS ix_price_book_items_scope_id;
...
ALTER TABLE price_book_items DROP COLUMN IF EXISTS scope_id;
...
DROP INDEX IF EXISTS ux_price_books_scope_pk;

DROP INDEX IF EXISTS ux_sites_scope_pk;
DROP INDEX IF EXISTS ux_machines_scope_pk;
...
DROP INDEX IF EXISTS ux_products_scope_pk;
