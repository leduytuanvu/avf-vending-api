...
| Production destructive E2E | Tracked template **`tests/e2e/.env.production.destructive.example`** → copy to **`tests/e2e/.env.production.destructive.local`** (gitignored). Requires **`E2E_ALLOW_DESTRUCTIVE=true`** and **`E2E_PRODUCTION_DESTRUCTIVE_CONFIRMATION=I_UNDERSTAND_DB_WILL_BE_RESET_AFTER_TEST`**. For read-only smoke with that file, export **`E2E_ALLOW_WRITES=false`** before the runner (overrides the file). See **`docs/archive/verification/production-full-destructive-e2e-final-report.md`**. |
