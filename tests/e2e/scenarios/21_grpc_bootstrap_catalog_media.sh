...
CHK_META="$(jq -nc --argjson meta "$META_BASE" --arg ik "$IK_CHECKIN" --arg ce "g21-ce-checkin-${RID}" \
  '$meta + {idempotencyKey:$ik, clientEventId:$ce}')"
CHK_BODY="$(jq -nc --argjson meta "$CHK_META" --arg boot "e2e-$(date +%s)" \
  '{meta:$meta, bootId:$boot, networkState:"online", attributes:{source:"e2e-grpc"}}')"
grpc_contract_try "$FLOW_ID" "check-in" MachineBootstrapService CheckIn "$CHK_BODY" "g21-checkin" "${IK_CHECKIN}" || ec=1

if [[ -n "$CFG_REV" ]] && [[ "$CFG_REV" =~ ^-?[0-9]+$ ]]; then
  ACKC_META="$(jq -nc --argjson meta "$META_BASE" --arg ik "$IK_ACKCFG" '$meta + {idempotencyKey:$ik}')"
  ACKC_BODY="$(jq -nc --argjson meta "$ACKC_META" --argjson cr "$CFG_REV" --arg pg "$PG_VER" \
    '{meta:$meta, acknowledgedConfigVersion:$cr, acknowledgedPlanogramVersionId:$pg}')"
  grpc_contract_try "$FLOW_ID" "ack-config" MachineBootstrapService AckConfigVersion "$ACKC_BODY" "g21-ack-config" "${IK_ACKCFG}" || ec=1
else
...
DELTA_BODY="$(jq -nc --arg m "$MID" --arg b "$BASIS" --argjson meta "$META_BASE" \
...
ACKCAT_META="$(jq -nc --argjson meta "$META_BASE" --arg ik "$IK_ACKCAT" '$meta + {idempotencyKey:$ik}')"
ACKCAT_BODY="$(jq -nc --argjson meta "$ACKCAT_META" --arg v "$ACK_CAT_VER" \
  '{meta:$meta, acknowledgedCatalogVersion:$v}')"
grpc_contract_try "$FLOW_ID" "ack-catalog-version" MachineCatalogService AckCatalogVersion "$ACKCAT_BODY" "g21-ack-cat" "${IK_ACKCAT}" || ec=1

MAN_BODY="$(jq -nc --arg m "$MID" --argjson meta "$META_BASE" '{machineId:$m, includeUnavailable:true, meta:$meta}')"
...
MMAN_BODY="$(jq -nc --arg m "$MID" --argjson meta "$META_BASE" '{machineId:$m, includeUnavailable:true, meta:$meta}')"
...
MDEL_BODY="$(jq -nc --arg m "$MID" --arg f "$MFP" --argjson meta "$META_BASE" \
...
ACKM_META="$(jq -nc --argjson meta "$META_BASE" --arg ik "$IK_ACKMED" '$meta + {idempotencyKey:$ik}')"
ACKM_BODY="$(jq -nc --argjson meta "$ACKM_META" --arg f "$MFP" '{meta:$meta, acknowledgedMediaFingerprint:$f}')"
grpc_contract_try "$FLOW_ID" "ack-media-version" MachineMediaService AckMediaVersion "$ACKM_BODY" "g21-ack-media" "${IK_ACKMED}" || ec=1

SCEN="21_grpc_bootstrap_catalog_media.sh"
if [[ "${GRPC_USE_REFLECTION:-false}" != "true" ]]; then
  log_docs_issue "P2" "$FLOW_ID" "$SCEN" "grpc-reflection" "gRPC" "${GRPC_ADDR:-}" "Proto-based grpcurl calls require documented GRPC_ADDR and import path when reflection is off" "Setup friction" "See docs for gRPC local dev" "${E2E_RUN_DIR}/grpc/g21-bootstrap.meta.json"
fi
