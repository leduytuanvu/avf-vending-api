...
-- TCN double-wide lane merge state (left + right slot codes per machine).
...
-- migrations/00031_machine_named_layouts.sql
CREATE TABLE machine_layouts (
    id uuid PRIMARY KEY DEFAULT public.uuid_generate_v7(),
    machine_id uuid NOT NULL REFERENCES machines (id) ON DELETE CASCADE,
    name text NOT NULL,
...
CREATE UNIQUE INDEX ux_machine_layouts_machine_name_active
    ON machine_layouts (machine_id, lower(btrim(name)))
    WHERE NOT is_archived;

CREATE INDEX ix_machine_layouts_machine ON machine_layouts (machine_id, updated_at DESC);

CREATE TABLE machine_layout_slots (
...
CREATE INDEX ix_machine_layout_slots_layout_ordinal ON machine_layout_slots (layout_id, slot_ordinal);

CREATE TABLE machine_layout_merge_pairs (
    id uuid PRIMARY KEY DEFAULT public.uuid_generate_v7(),
    layout_id uuid NOT NULL REFERENCES machine_layouts (id) ON DELETE CASCADE,
    left_slot_code text NOT NULL,
    right_slot_code text NOT NULL,
...
CREATE INDEX ix_machine_layout_merge_pairs_layout ON machine_layout_merge_pairs (layout_id);

CREATE TABLE machine_layout_device_state (
    machine_id uuid PRIMARY KEY REFERENCES machines (id) ON DELETE CASCADE,
...
CREATE TABLE machine_layout_snapshot_history (
    id uuid PRIMARY KEY DEFAULT public.uuid_generate_v7(),
...
CREATE INDEX ix_machine_layout_snapshot_history_machine_captured
    ON machine_layout_snapshot_history (machine_id, captured_at DESC);

CREATE INDEX ix_machine_layout_snapshot_history_machine_layout
    ON machine_layout_snapshot_history (machine_id, layout_id, captured_at DESC);

ALTER TABLE machines
    ADD COLUMN active_layout_id uuid REFERENCES machine_layouts (id) ON DELETE SET NULL,
    ADD COLUMN desired_active_layout_id uuid REFERENCES machine_layouts (id) ON DELETE SET NULL,
    ADD COLUMN reported_active_layout_id uuid REFERENCES machine_layouts (id) ON DELETE SET NULL;
