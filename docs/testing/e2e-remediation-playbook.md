...
- **`./tests/e2e/run-flow-review.sh`** always sets **`E2E_ALLOW_WRITES=false`**: **`--static-only`** never calls mutating APIs; **`--reuse-data`** only performs **read-only** GETs and optional **GetBootstrap** gRPC. You may still use it against **production** **only** when (a) companyal policy allows read probes, (b) tokens are **read-scoped**, and (c) responses are treated as sensitive. It does not remove other risks (e.g. logging PII); treat **`.e2e-runs/`** as secret-bearing.
