...
| WA-011 | Commerce reconciliation (read paths) | web-admin | REST | `/v1/admin/commerce/reconciliation` (per swagger) | (import from OpenAPI) | Company, date range, orders | Totals consistent | **scripted** (list in `10_reporting_audit_reconciliation.sh`) | read-only | None |
