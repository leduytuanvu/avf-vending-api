...
**Flow improvement review:** During runs, the harness may append **non-blocking** API/flow debt to **`improvement-findings.jsonl`** (see **`E2E_*`** toggles in **`tests/e2e/.env.example`**). Finalize produces **`improvement-summary.md`**, **`optimization-backlog.md`**, and **`flow-review-scorecard.json`** (mirrored at the run root). Use **`docs/testing/e2e-remediation-playbook.md`** to distinguish **failures** from **improvement findings** and P0–P3 handling.
