...
| FT-MAC-01 | P1 | Pilot machine activated; **Machine JWT** on device. **gRPC** `grpcs://…` reachable. Legacy machine HTTP **off** in prod. | (1) `MachineAuthService/ClaimActivation` (if new device) **or** refresh token if already bound. (2) `MachineBootstrapService/GetBootstrap` with valid meta. (3) Optionally `CheckIn` / `AckConfigVersion` if your flow requires. | **200 OK** equivalent: bootstrap shows **machineId**, **siteId**, topology/planogram hints per proto; **no** Admin JWT used on gRPC. | | | | |
