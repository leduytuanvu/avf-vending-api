...
| Interactive JWT | Explicit **`HTTP_AUTH_MODE` or `USER_JWT_MODE`**; explicit **`MACHINE_JWT_MODE`**; HS256 secrets ≥32 bytes, not documentation placeholders, not a single repeated byte; access TTL 5m-876000h (100y) and refresh TTL 1h-876024h (100y+1d) with access < refresh; revoke via logout / password change / credential_version / session revoke (JWT always has exp) |
