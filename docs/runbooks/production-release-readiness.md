...
- **Public API docs**: when `HTTP_SWAGGER_UI_ENABLED=true`, operators can open **`https://api.ldtv.dev/swagger/index.html`** (UI) and **`https://api.ldtv.dev/swagger/doc.json`** (OpenAPI JSON). That surface is documentation only; protected **`/v1`** traffic still needs Bearer JWTs.
