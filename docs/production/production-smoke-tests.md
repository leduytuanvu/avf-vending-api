...
Production deploys use [`deployments/prod/scripts/smoke_prod.sh`](../../deployments/prod/scripts/smoke_prod.sh) in **tiers** so safety is not based on `/health/ready` alone. JSON under `--json` is assembled by [`scripts/deploy/smoke/emit_production_smoke_json.py`](../../scripts/deploy/smoke/emit_production_smoke_json.py) for CI evidence.
