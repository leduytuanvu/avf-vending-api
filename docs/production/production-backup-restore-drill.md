...
2. A **repository file** checked out with the workflow, using the form:
   ...
   - Example: `path:deployments/docker/observability/evidence/2026-04/prod-backup.json`
...
- **Alternate**: commit JSON under a controlled path (e.g. `deployments/docker/observability/evidence/…`) and use `path:…` in `backup_evidence_id` (branch must be the one you run **Deploy Production** from, usually `main`).
