...
The script `scripts/deploy/monitoring/collect_deploy_slo_evidence.sh` runs on the GitHub Actions runner (not on paid APM) and writes **one JSON file per phase** under `deployment-evidence/`:
...
Use this as an operator checklist alongside `docs/deployment/two-vps-rolling-production-deploy.md` and `docs/production/production-smoke-tests.md`:
