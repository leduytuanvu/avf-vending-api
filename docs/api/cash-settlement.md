...
| GET | `/v1/admin/reports/cash` | Company cash collection report. Requires `from`/`to`; optional `site_id`, `machine_id`, `limit`, `offset`, and `format=csv`. CSV exports are audit logged. |

Related finance/reporting endpoints:

- `/v1/admin/reports/sales`
- `/v1/admin/reports/payments`
- `/v1/admin/reports/refunds`
- `/v1/admin/reports/inventory-low-stock`
- `/v1/admin/reports/machine-health`
- `/v1/admin/reports/failed-vends`
- `/v1/admin/reports/reconciliation-queue`

## Behavior

- **Company isolation**: Same as other admin machine routes (`company_id` query for `platform_admin`).
- **Report isolation**: Company-scoped report paths require the caller to be `platform_admin` or belong to the path company. `finance` and `finance_admin` include `reports.read`; `technician` does not.
