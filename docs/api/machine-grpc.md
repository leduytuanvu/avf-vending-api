...
| `MachineCatalogService` | **`GetSaleCatalog`** / **`SyncSaleCatalog`** (aliases of **`GetCatalogSnapshot`**), **`GetCatalogSnapshot`**, **`SyncCatalogBundle`** (delta catalog + flattened **`MediaAsset`** rows), **`GetCatalogDelta`**, **`AckCatalogVersion`**, **`GetMediaManifest`**. Slot-oriented sale projection — not the global product master. |
| `MachineProductMasterCatalogService` | **`GetProductMasterCatalogSnapshot`** (paginated full active product master), **`GetProductMasterCatalogDelta`** (versioned upserts + tombstones). Distinct from slot sale catalog. |
