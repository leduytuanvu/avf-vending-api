...
| `MachineCatalogService` | **`GetSaleCatalog`** / **`SyncSaleCatalog`** (aliases of **`GetCatalogSnapshot`**), **`GetCatalogSnapshot`**, **`SyncCatalogBundle`** (delta catalog + flattened **`MediaAsset`** rows), **`GetCatalogDelta`**, **`AckCatalogVersion`**, **`GetMediaManifest`**. |
