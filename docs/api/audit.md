...
Append-only `audit_events` rows include: `id`, `company_id`, `actor_type` (`user`, `machine`, `system`, `payment_provider`, `webhook`, `service`, …), optional `actor_id`, `action`, `resource_type`, optional `resource_id`, optional `machine_id`, optional `site_id`, `request_id`, `trace_id`, optional `ip_address` / `user_agent`, optional `before_json` / `after_json`, `metadata` (JSON), `outcome` (`success` / `failure`), `occurred_at`, `created_at`.
...
- `GET /v1/admin/audit/events` — legacy/global-style URL; org scope from JWT or `company_id` query for platform admins.
- `GET /v1/admin/audit-events` — list with filters: `action`, `actorId` / `actor_id`, `actorType` / `actor_type`, `outcome`, `resourceType` / `resource_type`, `resourceId` / `resource_id`, `machineId` / `machine_id`, `from`, `to`, `limit`, `offset`.
- `GET /v1/admin/audit-events/{auditEventId}` — fetch one row (404 if missing or outside org).

Requires **audit.read** (`audit:read`). Cross-company access is denied at the HTTP scope layer.
