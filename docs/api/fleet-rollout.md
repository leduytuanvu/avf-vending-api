...
Role-scoped admin APIs drive **bulk machine provisioning** (batch metadata + optional activation codes) and **MQTT-backed fleet rollouts** (`fleet_rollout_apply` via the command ledger).

## Bulk provisioning

| Method | Path | Permission |
| --- | --- | --- |
| `POST` | `/v1/admin/provisioning/machines/bulk` | `fleet:write` |
| `GET` | `/v1/admin/provisioning/batches/{batchId}` | `fleet:read` |
...
| `site_ids`, `statuses`, `model` | Filter company machines (`model` is substring match). |
| `tag_ids` | Machine must have **all** listed catalog tag UUIDs (`machine_tag_assignments`). |
| `tag_slugs` | Same as `tag_ids`, resolved case-insensitively on `tags.slug` within the company. Unknown slug → `400 invalid_argument`. |
...
All under `/v1/admin/rollouts…`, `fleet:write` except `GET` (`fleet:read`):
...
Role-scoped listing and operator retry/cancel (mounted on the company admin router):

| Method | Path | Permission |
| --- | --- | --- |
| `GET` | `/v1/admin/commands` | `fleet:read` |
| `GET` | `/v1/admin/commands/{commandId}` | `fleet:read` |
| `POST` | `/v1/admin/commands/{commandId}/retry` | `device_commands:write` |
| `POST` | `/v1/admin/commands/{commandId}/cancel` | `device_commands:write` |
