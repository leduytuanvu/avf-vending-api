...
`scripts/test/run-readonly-smoke.sh` honors **`APP_ENV`**, **`SMOKE_EXPECT_OPENAPI_JSON`**, **`SMOKE_EXPECT_PUBLIC_METRICS`**, **`OPS_BASE_URL`**, and **`METRICS_SCRAPE_TOKEN`**. It writes **`reports/test/readonly-smoke.json`** and **`reports/test/readonly-smoke.md`** with **redacted** body snippets (tokens are never printed).
