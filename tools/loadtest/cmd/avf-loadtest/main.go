...
func main() {
	...
	case "admin":
		if strings.TrimSpace(*adminJWT) == "" {
			fmt.Fprintln(os.Stderr, "admin scenario needs -admin-jwt")
			os.Exit(2)
		}
		rec := &loadtest.LatencyRecorder{}
		start := time.Now()
		...
		for startAt := time.Now(); time.Since(startAt) < *duration; {
			...
			if err := loadtest.RunHTTPAdminSequence(deadline, *httpBase, *adminJWT, 1, rec); err != nil {
				fmt.Fprintln(os.Stderr, err)
				os.Exit(1)
			}
		}
