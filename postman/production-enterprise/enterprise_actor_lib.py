...
def market_folder(flow: dict[str, Any], classification: str) -> str:
    """Map manifest flow to expanded market-ready folder tree."""
    fid = str(flow.get("id") or "")
    phase = str(flow.get("phase") or "")
    path = str(flow.get("path") or "")
    ...
    if classification == "ONLINE_PAYMENT_EXCLUDED":
        if "webhook" in path or "refund" in path:
            return "97 - Online Payment Guarded/Payment webhook guarded"
        ...
        if "vietqr" in path.lower() or "qr" in path.lower():
            return "97 - Online Payment Guarded/VietQR guarded"
        return "97 - Online Payment Guarded/Payment reconciliation guarded"

    if classification in ("OPTIONAL_SKIPPED_BY_PRODUCTION_CONTRACT", "CONFIG_REQUIRED"):
        if "media/uploads" in path or "presigned" in fid.lower():
            return "98 - Optional Contract Disabled/Presigned media upload"
    ...
    if phase == "preflight":
        if "live" in path:
            return "01 - Public Health Monitoring/Health live"
        if "ready" in path:
            return "01 - Public Health Monitoring/Health ready"
        return "01 - Public Health Monitoring/Version"

    if phase == "auth":
        if "login" in fid or "login" in path:
            return "02 - Auth Session RBAC/Admin login"
        if fid == "REST-AUTH-002":
            return "02 - Auth Session RBAC/Admin current user"
        return "02 - Auth Session RBAC/Negative auth"

    if phase == "catalog":
        label = str(flow.get("label") or "").lower()
        if "categor" in label:
            return "04 - Admin Catalog Foundation/Categories CRUD"
        if "brand" in label:
            return "04 - Admin Catalog Foundation/Brands CRUD"
        if "tag" in label:
            return "04 - Admin Catalog Foundation/Tags CRUD"
        return "05 - Admin Product Lifecycle/Product CRUD"

    if phase == "media":
        return "06 - Admin Media Image Pipeline/Cloudinary direct backend upload"

    if phase == "site":
        return "07 - Site Location Machine Provisioning/Create site location"
    if phase in ("machine", "activation"):
        if "activation" in path:
            return "07 - Site Location Machine Provisioning/Activation code create"
        return "07 - Site Location Machine Provisioning/Create machine"

    if phase == "topology":
        return "09 - Topology Planogram Slot Mapping/Create update topology"
    if phase == "planogram":
        return "09 - Topology Planogram Slot Mapping/Publish planogram"
    if phase == "stock":
        return "10 - Stock Inventory Refill/Restock"
    if phase == "operator":
        return "11 - Technician Operator Workflows/Operator session start"

    if phase == "report":
        if "fleet" in path:
            return "15 - Reports Analytics Reconciliation/Fleet health report"
        if "sales" in path:
            return "15 - Reports Analytics Reconciliation/Sales report"
        if "inventory" in path:
            return "15 - Reports Analytics Reconciliation/Inventory report"
        return "15 - Reports Analytics Reconciliation/Machine activity report"

    if phase == "commerce" and classification != "ONLINE_PAYMENT_EXCLUDED":
        return "14 - Commerce No-Online-Payment/Cash manual order"

    if phase == "negative":
        return "18 - Security Negative Abuse/Missing token"

    if phase == "cleanup":
        return "19 - Cleanup Reconciliation/Delete cleanup E2E products"

    if phase == "rest-coverage":
        rm = (flow.get("route_matrix") or {}).get("coverage") or ""
        if rm == "documented_skip":
            return "17 - Route Coverage Smoke/Documented skips"
        if path.startswith("/v1/reports"):
            return "17 - Route Coverage Smoke/Reports route coverage"
        if path.startswith("/v1/machines/"):
            return "17 - Route Coverage Smoke/Machine route coverage"
        if rm == "auth_negative":
            return "18 - Security Negative Abuse/Wrong role"
        return "17 - Route Coverage Smoke/Admin readonly route coverage"

    if fid.startswith("REST-FLOW-"):
        return "90 - Full Market Release Flows/Flow 90.01 Admin creates sellable product with image"

    if phase == "audit" or "/audit" in path:
        return "16 - Audit Events Logs Diagnostics/Admin audit logs"
