...
Provisioned dashboards (file provider) live under `deployments/docker/observability/grafana/provisioning/dashboards/json/`:
...
Datasource UIDs: `prometheus`, `loki` (see `deployments/docker/observability/grafana/provisioning/datasources/datasource.yml`).
...
- **ClickHouse:** optional cold-path only. When `ANALYTICS_CLICKHOUSE_ENABLED=true` and `ANALYTICS_MIRROR_OUTBOX_PUBLISHED=true`, `cmd/worker` exposes:
  ...
  See `deployments/docker/observability/ANALYTICS_CLICKHOUSE.md`. Compose `experimental` profile runs ClickHouse for local trials.
...
For local/lab Docker, see [`deployments/docker/observability/prometheus/prometheus.yml`](prometheus/prometheus.yml).
