...
Use **Grafana + Prometheus** for first-line panels when `METRICS_ENABLED` is on (`deployments/docker/observability/grafana/provisioning/dashboards/json/`, `deployments/docker/observability/METRICS.md`). Supplement with **JSON logs** (Loki / CloudWatch Logs Insights / etc.) for context and fields not exported as series.
