...
- **Smoke:** `scripts/deploy/smoke_staging.sh` wraps the top-level [scripts/deploy/smoke_staging.sh](../../scripts/deploy/smoke_staging.sh) and can derive `STAGING_BASE_URL` from `API_DOMAIN` in `.env.staging`
