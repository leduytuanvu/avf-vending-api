...
if [[ "${RESTORE_MODE}" == "validate" ]]; then
	...
	if [[ -n "${REPORT_PATH}" ]]; then
		...
		printf '  "restore_target_identifier": "%s",\n' "$(json_escape "${restore_target_identifier}")" >>"${REPORT_PATH}"
...
if [[ -n "${REPORT_PATH}" ]]; then
	...
	printf '  "restore_drill_executed": %s,\n' "$([[ "${RESTORE_MODE}" == "drill" ]] && printf 'true' || printf 'false')" >>"${REPORT_PATH}"
	printf '  "mode": "%s",\n' "${RESTORE_MODE}" >>"${REPORT_PATH}"
	printf '  "env_file_path": "%s",\n' "$(json_escape "${ENV_FILE_PATH}")" >>"${REPORT_PATH}"
	printf '  "dump_path": "%s",\n' "$(json_escape "${DUMP_PATH}")" >>"${REPORT_PATH}"
	printf '  "database_url_kind": "%s",\n' "${database_url_kind}" >>"${REPORT_PATH}"
	...
	printf '  "restore_target_identifier": "%s",\n' "$(json_escape "${restore_target_identifier}")" >>"${REPORT_PATH}"
	if [[ "${RESTORE_MODE}" == "drill" ]]; then
		printf '  "target_database_name": "%s",\n' "$(json_escape "${target_database_name}")" >>"${REPORT_PATH}"
		printf '  "non_system_table_count": %s,\n' "${non_system_table_count}" >>"${REPORT_PATH}"
		printf '  "dump_table_count": %s,\n' "${dump_table_count}" >>"${REPORT_PATH}"
	fi
	printf '  "verification_steps_executed": %s,\n' "${verification_steps}" >>"${REPORT_PATH}"
	printf '  "summary": "%s",\n' "$(json_escape "${summary}")" >>"${REPORT_PATH}"
	printf '  "verdict": "pass"\n' >>"${REPORT_PATH}"
	printf '}\n' >>"${REPORT_PATH}"
fi
