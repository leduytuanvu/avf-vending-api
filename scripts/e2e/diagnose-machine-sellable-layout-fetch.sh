...
DIAG_MACHINE_ID="${1:-}"
ARTIFACT_DIR="${2:-}"
if [[ -z "$DIAG_MACHINE_ID" || -z "$ARTIFACT_DIR" ]]; then
...
# load_env may set MACHINE_ID/TEST_MACHINE_ID from .env — always pin to CLI arg.
export MACHINE_ID="${DIAG_MACHINE_ID}"
export TEST_MACHINE_ID="${DIAG_MACHINE_ID}"
export E2E_TEST_MACHINE_ID="${DIAG_MACHINE_ID}"
unset MACHINE_TOKEN MACHINE_ACCESS_TOKEN

eval "$(bash scripts/e2e/mint-production-machine-token.sh "${DIAG_MACHINE_ID}")"
export MACHINE_ID="${DIAG_MACHINE_ID}"
export TEST_MACHINE_ID="${DIAG_MACHINE_ID}"
export E2E_TEST_MACHINE_ID="${DIAG_MACHINE_ID}"
...
meta="$(jq -nc --arg mid "${DIAG_MACHINE_ID}" --arg rid "diag-layout-${E2E_RUN_TS}" '{machineId:$mid,requestId:$rid}')"
...
if ADMIN_TOK="$(e2e_admin_token 2>/dev/null)"; then
  e2e_curl_get "admin-machine" "${BASE_URL}/v1/admin/machines/${DIAG_MACHINE_ID}" "$ADMIN_TOK" >/dev/null || true
  e2e_curl_get "admin-inventory" "${BASE_URL}/v1/admin/machines/${DIAG_MACHINE_ID}/inventory" "$ADMIN_TOK" >/dev/null || true
  e2e_curl_get "admin-planograms" "${BASE_URL}/v1/admin/planograms?machineId=${DIAG_MACHINE_ID}&limit=5" "$ADMIN_TOK" >/dev/null || true
fi

export TEST_MACHINE_ID MACHINE_ACCESS_TOKEN MACHINE_ID
...
e2e_grpc_call "avf.machine.v1.MachineCatalogService/GetCatalogSnapshot" \
  "$(jq -nc --arg mid "${DIAG_MACHINE_ID}" --argjson meta "$meta" '{machineId:$mid,includeUnavailable:false,includeImages:true,meta:$meta}')" \
  "grpc-catalog-sellable" machine "" || true

e2e_grpc_call "avf.machine.v1.MachineCatalogService/GetCatalogSnapshot" \
  "$(jq -nc --arg mid "${DIAG_MACHINE_ID}" --argjson meta "$meta" '{machineId:$mid,includeUnavailable:true,includeImages:true,meta:$meta}')" \
  "grpc-catalog-all" machine "" || true

e2e_grpc_call "avf.machine.v1.MachineInventoryService/GetPlanogram" \
  "$(jq -nc --arg mid "${DIAG_MACHINE_ID}" '{machineId:$mid}')" "grpc-planogram" machine "" || true
...
if [[ -n "${MACHINE_ACCESS_TOKEN:-}" ]]; then
  curl -sS -o "${ARTIFACT_DIR}/raw/rest-sale-catalog.body" -w '%{http_code}' \
    ...
    -H "x-machine-id: ${DIAG_MACHINE_ID}" \
    "${BASE_URL}/v1/machines/${DIAG_MACHINE_ID}/sale-catalog?include_images=true" \
    >"${ARTIFACT_DIR}/raw/rest-sale-catalog.meta" 2>/dev/null || true
fi

echo "FETCH_OK artifact_dir=${ARTIFACT_DIR} machine_id=${DIAG_MACHINE_ID}"
