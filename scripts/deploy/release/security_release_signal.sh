...
emergency_on_missing() {
  ...
  python3 scripts/deploy/security/write_security_verdict.py emergency --emergency-reason "Exit trap: verdict missing or invalid (Security Release signal step ended before a successful contract write)"
  return 0
}
trap 'emergency_on_missing' EXIT
if [[ ! -f scripts/deploy/security/write_security_verdict.py ]]; then
  echo "::error::expected scripts/deploy/security/write_security_verdict.py from checkout" >&2
  exit 1
fi
emit_verdict_outputs_and_summary() {
  set +e
  python3 scripts/deploy/security/emit_security_verdict_outputs.py >> "$GITHUB_OUTPUT"
  _eo=$?
  python3 scripts/deploy/security/emit_security_verdict_summary.py --section main >> "$GITHUB_STEP_SUMMARY"
  _es=$?
  set -e
  if [[ "${_eo}" -ne 0 ]]; then
...
emit_source_debug_table_safe() {
  set +e
  python3 scripts/deploy/security/emit_release_signal_debug_table.py
  _ed=$?
  set -e
  if [[ "${_ed}" -ne 0 ]]; then
...
if [[ "${SKIP_R}" == "success" ]]; then
  if [[ "${EVENT_NAME}" == "workflow_run" ]]; then
    ...
    if [[ "${te}" == "workflow_run" ]]; then
      ...
      python3 scripts/deploy/security/write_security_verdict.py unsupported-trigger
      ...
      python3 scripts/deploy/security/write_security_verdict.py skipped
...
if [[ "${NO_RC}" -eq 1 ]]; then
  python3 scripts/deploy/security/write_security_verdict.py no-candidate
  emit_verdict_outputs_and_summary
  {
    echo ""
...
if [[ -n "${META_CONFLICT// }" ]]; then
  ...
  python3 scripts/deploy/security/write_security_verdict.py metadata-mismatch
  emit_verdict_outputs_and_summary
  trap - EXIT
  exit 0
fi
...
if [[ -n "${A_EV// }" && "${A_EV}" != "push" && "${A_EV}" != "workflow_dispatch" && "${A_EV}" != "manual" ]]; then
  python3 scripts/deploy/security/write_security_verdict.py unsupported-artifact-source-event
  emit_verdict_outputs_and_summary
  trap - EXIT
  exit 0
fi
if [[ -z "${want_branch// }" || ( "${want_branch}" != "develop" && "${want_branch}" != "main" ) ]]; then
  python3 scripts/deploy/security/write_security_verdict.py ineligible-branch
  emit_verdict_outputs_and_summary
  trap - EXIT
  exit 0
fi
...
set +e
python3 scripts/deploy/security/write_security_verdict.py full
_vw=$?
set -e
if [[ "${_vw}" -ne 0 ]]; then
  echo "::error::write_security_verdict.py full mode failed; writing emergency fail verdict" >&2
  set +e
  python3 scripts/deploy/security/write_security_verdict.py emergency --emergency-reason "write_security_verdict.py full exited with code ${_vw}"
  set -e
fi
emit_verdict_outputs_and_summary
emit_source_debug_table_safe
