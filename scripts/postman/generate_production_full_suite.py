...
GATED_WRITE_PREREQUEST = [
    ...
    "const readiness = String(pm.environment.get('readiness')).toLowerCase() === 'true';",
    "const legacyGate = allowDestructive || canaryMode || readiness;",
    "const method = (pm.request.method || '').toUpperCase();",
    "const isWrite = ['POST','PUT','PATCH','DELETE'].indexOf(method) >= 0;",
    "const reqName = pm.info.requestName || '';",
    "if (isWrite && reqName.indexOf('[GATED-WRITE]') >= 0) {",
    ...
    "    }",
    "  }",
    "  if (reqName.indexOf('[DESTRUCTIVE]') >= 0 && !allowDestructive) {",
    "    throw new Error('Destructive request blocked: set allow_destructive=true');",
    "  }",
    "  if (reqName.indexOf('[CANARY]') >= 0 && !canaryMode) {",
...
LOGIN_TEST_EXTRA = [
    ...
    "    if (json.accountId) pm.environment.set('accountId', json.accountId);",
    "  } catch (e) { /* ignore */ }",
    "}",
]
...
def patch_headers(headers: list[dict], method: str, is_gated: bool) -> list[dict]:
    out: list[dict] = []
    seen: set[str] = set()
    for h in headers or []:
        if h.get("disabled"):
    ...
    for key, val in (
        ...
        if key.lower() not in seen:
            ...
            seen.add(key.lower())
    if method in ("POST", "PUT", "PATCH", "DELETE") and "idempotency-key" not in seen:
...
def patch_request_item(item: dict) -> dict:
    ...
    is_gated = "[GATED-WRITE]" in name or method in ("POST", "PUT", "PATCH", "DELETE")
    is_login = "/v1/auth/login" in request_path_key(item).lower()
    ...
    req["header"] = patch_headers(req.get("header") or [], method, is_gated)
    ...
    for ev in events:
        if ev.get("listen") == "prerequest":
            has_prerequest = True
            script = GATED_WRITE_PREREQUEST if (is_gated and not is_login) else READONLY_PREREQUEST
            ...
            if is_login:
                exec_lines = [ln for ln in exec_lines if "v1/auth/login" not in ln or "tokens.accessToken" in ln]
                exec_lines.extend(LOGIN_TEST_EXTRA)
        ...
        new_events.append(
            {
                ...
                "script": {
                    ...
                    "exec": LOGIN_TEST_EXTRA if is_login else ["pm.test('Status not 500', function(){ pm.expect(pm.response.code).to.not.equal(500); });"],
                },
            }
        )
